            ...
            <div class="bg-white dark:bg-gray-800 relative overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 border sm:rounded-lg">
                    ...
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <a href="{{ route('product.create') }}" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                    ...
                    @forelse($datasets as $dataset)
                        <div class="p-4 border rounded-lg">
                            <div class="flex items-center space-x-4">
                                <img src="{{ asset('storage/' . $dataset->image) }}" class="w-20 h-20 rounded object-center object-cover" alt="">
                                <div>
                                    <div class="text-lg font-medium">{{ $dataset->name }}</div>
                                    <div class="text-base mt-1 text-primary-600">Rp. {{ $dataset->price }} {{ $dataset->unit ? ' / ' . $dataset->unit: null }}</div>
                                </div>
                            </div>
                                    ...
                                    <div id="deleteModal{{ $dataset->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                        <div class="flex items-center justify-center relative p-4 w-full max-w-md h-full md:h-auto">
                                            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                                ...
                                                <form action="{{ route('product.destroy', $dataset) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
